...
function UserPwdDeleteModal({pwdopen, setPwdOpen, clickid, user, setUser}){
      ...
      .then(res => {
        if (res.data && res.data.success) {
          ...
          setTimeout(() => setDeleteAlert({ ...deletealert, success: false }), 2000); // 2초 후 deletealert 상태 초기화
            ...
            axios.delete(`${process.env.REACT_APP_SERVER_URL}licenseUser/api/deleteUser/${clickid}`)
            .then(res => {
              setUser(user.filter((item) => item.id !== clickid))
            })
    ...
    return(
      ...
      >
        ...
        <div className="modalbutton"> 
          <button className="editsubmit" onClick={handlePwd}> 확인 </button>
        </div>
        </Box>
      </Modal>
    )
...
export default UserPwdDeleteModal;